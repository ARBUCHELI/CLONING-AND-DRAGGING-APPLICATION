...
rep.forEach(function(ele, ind) { //Select all elements and loop through them.  This function gets the element, the index value of each element//
	...
	ele.addEventListener("click",function(e){ //This makes all the selected elements clickable//
		...
		holder.obj.style.backgroundColor = ranColor(); //This calls a function called random color.
		...
		holder.obj.startX = ele.offsetLeft; //These are going to be numeric values, we don't need to parse them as integers.
	...
	ele.style.left = ind*160 + "px"; //This allows to position the boxes in the screen with a separation that depends of the index//
})
